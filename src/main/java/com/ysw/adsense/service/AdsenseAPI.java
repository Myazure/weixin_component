...
public class AdsenseAPI {
	...
	public String getAuthorizerAccessTokenStr(String authorizer_appid) {
		String key = AdsenseAPI.genAuthorizerAccessTokenKey(authorizer_appid);
		// Fetch from redis first
		LOG.info("[YSW Adsense]: Get >>>Authorizer Access Token<<< from redis.");
	...
	public String getAuthorizerRefreshTokenStr(String authorizer_appid) {
		...
		if (null == authorizerRefreshToken || authorizerRefreshToken.trim().length() == 0) {
			...
			AdOfficialAccount oa = officialAccountService.findByAppId(authorizer_appid);
	 ...
	 * @return
	 */
	public List<AdWxUser> getAppUsers(String appidString) {
		List<AdWxUser> resault = new ArrayList<AdWxUser>();
		if (checkUserManagementAccess(appidString)) {
			LOG.info(appidString + ":checkUserManagementAccess SUCCESS!!!!");
		} else {
			LOG.info("appid:" + appidString + ">>>>>checkUserManagementAccess Faild!!!! No UserManagementAccess");
			return resault;
		}
		String access_token = getAuthorizerAccessTokenStr(appidString);
		if (access_token == null) {
			LOG.info(access_token + ":>>>>>>>access_token Faild!!!! No access_token appid:" + appidString);
			return resault;
		}
		List<String> openidList = new ArrayList<String>();
		FollowResult users = UserAPI.userGet(access_token, null);
		if (users.getTotal() == null | users.getCount() == null) {
			return resault;
		}
		if (users.getTotal() == 0 | users.getCount() == 0) {
			return resault;
		}
		while (users.getNext_openid() != null) {
			if (users.getData() == null) {
				break;
			}
			if (users.getData().getOpenid() == null) {
				break;
			}
			for (int i = 0; i < users.getData().getOpenid().length; i++) {
				openidList.add(users.getData().getOpenid()[i]);
			}
			users = UserAPI.userGet(access_token, users.getNext_openid());
		}
		return resault;
	}
